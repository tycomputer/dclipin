...
<%@page import="com.tycomputer.hj.manager.action.PresentForm"%>
...
<ty:validationTag formId="hjpresent"/>
<script type="text/javascript">
	function toList(){
		document.location.href = "hjmanager/hjpresent.do";
		return false;
	}
	function submitForm(){
		if ($F("pname") == ""){
	...
	function init(){
		...
		$("pictd").appendChild(Builder.node('img',{src:'images/h/<s:property value="#request.form.bigpic"/>',border:"0"}));
		</s:if>
	}
...
<body onload="init();">
	<table width="100%" class="overHidden" border="1" cellpadding="3" cellspacing="0">
		...
		<s:form action="hjpresent!savePresent" enctype ="multipart/form-data">
		<tr>
			<td>名称</td>
			<td>				
		...
		<tr>
			<td>标志</td>
			<td>
				<s:select cssClass="required" list="#{0:'正常',1:'停产'}" name="form.flag" id="form.flag"></s:select> 停产的货架不会在用户看到的页面上显示，只有正常的才显示
			</td>
		</tr>
		<tr>
			<td>目录</td>
			<td>				
				<ty:sqlSelect name="form.cataId" emptyOption="true" sql="select t.cataId,t.cataName from Hjdatacata t " onchange="typeChange();"/>
			</td>
		</tr>
		<tr>
			<td>类别</td>
	...
	//<tr>
		...
		<tr>
			<td>图片</td>
			<td id="pictd"><s:file name="pic" id="pic" />最好的尺寸是：394*394</td>
		</tr>
		<tr>
			<td>简介</td>
			<td>
