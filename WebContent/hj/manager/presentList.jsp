...
<script>
	function toConfirm(){
		...
		var _AJAX_PAGE_CELLFUNCS_DEFAULT = [ 
			...
			function(data) { return (data[3]==0) ? "正常" : "停产"; },
			function(data) { return data[4]; },
			function(data) { return "<a href='hjmanager/hjpresent!toEdit.do?form.uid=" + data[5] + "'>修改</a>&nbsp;&nbsp;<a onclick='return ask();' href='hjmanager/hjpresent!toDele.do?form.uid=" + data[5] + "'>删除</a>" ; }
		];
		function ask(){
		...
		function init(){
			...
			<s:if test="#request.myuid != null">
				window.open('../hjDetail.do?i=<s:property value="#request.myuid"/>');
			</s:if>
		}
</script>
</head>
<body onload="init();">
	<table width="100%" class=overHidden border="1" cellpadding="3" cellspacing="0">
		<caption>货架管理</caption>
	<tr>
	
		<td colspan="8">			
			<s:form action="hjpresent">
				<a href="hjmanager/hjpresent!addPresent.do" >添加货架</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="hj/manager/managerMenu.jsp">返回</a>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				目录：<ty:sqlSelect name="form.cataId" emptyOption="true" sql="select t.cataId,t.cataName from Hjdatacata t " onchange="typeChange();"/>
		...
		<ty:ajaxPage sql="sql"  size="20" 
					colwidths="50,50,80,20,30,60" genCellFuncs="false"
					heads="目录,类别,名称,标志,排序号,操作" />
	</table>
</body>
</html>