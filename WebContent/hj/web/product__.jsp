...
<script>
	...
	var tid,did,pid,tstr,dstr,pstr,timer,extime;
	var initFlag = true;
	...
	function setTimer(){
		...
		timer = new PeriodicalExecuter(function(pe) {
			if (extime == 0){
				...
				$("timeButton").value = extime + " 秒搜索，点击立即搜索";
				extime--;
			}			
		}, 1);
		return false;
	...
	function show(data){
		for (var i=0; i<data.size(); i++){
		    ...
		    var dl = Builder.node("dl",[
			    						   ...
			    						   Builder.node("dd",[Builder.node("a",{href:"presentDetail.do?i="+p.i},[p.n,Builder.node("br"),p.i])])
			    						   ]);
	...
	function showPresent(){
		...
		new Ajax.Request("sp", {parameters: {t:tid,d:did,p:pid,w:$F(w)},
			onCreate:function(t){$("_nav").innerHTML="信息加载中，请稍等...";$("_btnnav").innerHTML="信息加载中，请稍等...";},			
			onSuccess: function(t) {
				var json = t.responseText.evalJSON();
				$("spDiv").innerHTML="";
