...
public class ShowPresentServlet extends HttpServlet {
	...
	private static final long serialVersionUID = -36616769965319881L;
...
//			}
		...
		if (bean.getSql() == null) {
			StringBuffer sb = new StringBuffer("select t.uid,t.pname,t.price,t.litpic from Hjpresent t where t.flag='0' ");
			...
			if ((bean.getWords() != null) && (!bean.getWords().trim().equals(""))) {
				...
				for (int i = 0; i < w.length; i++) {
					if ((w[i] != null) && (!w[i].trim().equals(""))) {
						sb.append(" and (t.pname like '%").append(w[i]).append("%' or t.words like '%").append(w[i]).append("%' or t.description like '%").append(w[i]).append("%') ");
					}
				}
			}
