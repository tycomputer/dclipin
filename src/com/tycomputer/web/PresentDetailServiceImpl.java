...
public class PresentDetailServiceImpl implements IPresentDetailService {
	...
	public Present getPresent(String id) {
		Present present = (Present) dao.load(Present.class, id);
		if (present != null) {
			java.text.DecimalFormat df = new java.text.DecimalFormat("#.00");
			present.setPriceStr(df.format(present.getPrice()));
			// if (present.getDescription() != null){
			// present.setDescriptionStr(present.getDescription().getValue());
			// }
		}
		return present;
	}
	...
	public String getPresentListJSON(String id) {
		...
		List l = dao
				.find("select t.uid,t.datacata.cataName,t.datatype.typeName,t.pname,t.price,t.description,t.bigpic,t.unit,t.spec,t.buynum,t.itsize,t.packsize,t.weight,t.material,t.resale from Present t where t.flag='0' and t.datatype=(select a.datatype from Present a where a.uid=?) order by t.ordersn,t.addData desc",
						new Object[] { id });
		if (l != null) {
	...
	private String htmlParse(final String html) {
		...
		try {
			...
			for (int i = 0; i < nodes.size(); i++) {
				...
				sb.append(new String(nodet.toPlainTextString()));// .getBytes(
				// "UTF-8")
				i++;
			}
		} catch (Exception e) {
