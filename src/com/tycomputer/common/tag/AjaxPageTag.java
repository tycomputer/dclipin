...
//		pageBean.setDbId(dbId);
	...
	public String genJavaScript() {
		...
		// 生成TR
		sb.append("function _genTR(data){var arr=$A();var i=0;_AJAX_PAGE_CELLFUNCS").append(id).append(".each(function (f){arr[i]=f(data);i++;});var tr=Builder.node('tr');arr.each(function(a){var td=Builder.node('td',{innerHTML:a});tr.appendChild(td);});return tr;}\n");
		// 生成ajax访问函数
